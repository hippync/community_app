-- ========================================
-- SCRIPT COMPLET SUPABASE - VERSION AVEC RLS
-- Création de la table + Sécurité RLS activée
-- ========================================
...
DROP POLICY IF EXISTS "allow_authenticated_read" ON manifestations_interet;
...
ALTER TABLE manifestations_interet ENABLE ROW LEVEL SECURITY;

-- ÉTAPE 5 : Politique d'insertion pour utilisateurs anonymes
-- Permet à n'importe qui de soumettre le formulaire
-- La contrainte UNIQUE empêchera les doublons d'email
...
-- ÉTAPE 6 : Politique de lecture pour utilisateurs authentifiés uniquement
-- Seuls les admins authentifiés peuvent voir les données
...
-- ========================================
-- NOTES DE SÉCURITÉ AVEC RLS
-- ========================================

-- ✅ RLS activé
-- ✅ Insertions : Autorisées pour utilisateurs anonymes (formulaire public)
-- ✅ Lecture : Réservée aux utilisateurs authentifiés (admin)
-- ✅ Mise à jour/Suppression : Bloquées pour tous (pas de politique)
-- ✅ Email unique : Géré par contrainte UNIQUE (empêche doublons)
-- ✅ Validation : role doit être 'volunteer', 'nonprofit', ou 'business'

-- AVERTISSEMENT :
-- Si vous obtenez des erreurs 401 avec ce script, utilisez plutôt
-- supabase_security.sql (sans RLS) pour une configuration fonctionnelle.
